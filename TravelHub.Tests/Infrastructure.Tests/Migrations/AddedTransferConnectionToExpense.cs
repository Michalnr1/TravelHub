...
public class AddedTransferConnectionToExpenseMigrationTests
{
    private readonly Assembly _migrationsAssembly;

    public AddedTransferConnectionToExpenseMigrationTests()
    {
        _migrationsAssembly = typeof(ApplicationDbContext).Assembly;
    }

    [Fact]
    public void Up_ShouldAddTransferredToColumnAndForeignKey()
    {
        // Arrange
        ...
        Assert.Equal("TransferredToId", addColumnOperation.Name);
        Assert.Equal("Expenses", addColumnOperation.Table);
        Assert.Equal("nvarchar(450)", addColumnOperation.ColumnType);
        ...
        Assert.Equal("Expenses", createIndexOperation.Table);
        Assert.Equal("TransferredToId", createIndexOperation.Columns[0]);
        ...
        Assert.Equal("Expenses", addForeignKeyOperation.Table);
        Assert.Equal("TransferredToId", addForeignKeyOperation.Columns[0]);
        Assert.Equal("AspNetUsers", addForeignKeyOperation.PrincipalTable);
    }
    ...
    public void Down_ShouldRemoveForeignKeyAndColumn()
    {
        ...
        Assert.Equal("Expenses", dropForeignKeyOperation.Table);
        ...
        Assert.Equal("Expenses", dropIndexOperation.Table);
        ...
        Assert.Equal("TransferredToId", dropColumnOperation.Name);
        Assert.Equal("Expenses", dropColumnOperation.Table);
    }
    ...
    private object CreateMigrationInstance()
    {
        var migrationType = _migrationsAssembly.GetTypes()
            .FirstOrDefault(t => t.Name == "AddedTransferConnectionToExpense");

        if (migrationType == null)
        {
            throw new InvalidOperationException("AddedTransferConnectionToExpense migration type not found");
        }
